...
import com.sss.cmdconsole.builtincmd.CmdSettings.Setting;
import com.sss.cmdconsole.builtincmd.FileSys;
import com.sss.cmdconsole.builtincmd.CmdSettings;
import com.sss.cmdconsole.builtincmd.HelpCmd;
import com.sss.cmdconsole.builtincmd.SystemCmd;
...
public class CommandDispatcher implements Runnable, IStdOut
{
	 ...
	 private void preCommandLoop(CmdSettings cmdSettings, FileSys fileSys)
	 {
		 ...
		 while (lsFullMode != fileSys.toggleListDirFullMode()) { } // toggle until same as saved value
		...
		while (isRun) {
			...
			if (builtInCmd == CmdInfo.BuiltInCmd.ls) {
			   if (args != null) {
				  if (args.length >= 3) {
					 ...
					 if (args[1].equalsIgnoreCase("-f")) {
						boolean isFullMode = fileSys.toggleListDirFullMode();
						cmdSettings.setValue(CmdSettings.Setting.ls_full_mode, isFullMode);
						writeln(CFunc.getString(isFullMode ? R.string.ls_fullmode_on : 
							                                 R.string.ls_fullmode_off));
					 }
					 else {
					    String dirPath = FileSys.getResolvedPath(fileSys.getResultantPath(args[1]));
					    if (!FileSys.isDir(dirPath)) {
					    	printError(dirPath + CFunc.getString(R.string.error_not_dir));
					    }
					    else {
			   ...
			   fileSys.printCurDir();
				...
				if (args != null && args.length >= 2) {
					String apkPath = FileSys.getResolvedPath(fileSys.getResultantPath(args[1]));
				...
				LinkedHashMap<Integer, String> cmdHistoryList = cmdSettings.getHistoryList(0);
				if (cmdHistoryList != null && cmdHistoryList.size() > 0) {
					Iterator<Map.Entry<Integer, String> > it = cmdHistoryList.entrySet().iterator();
					while (it.hasNext()) {
				...
				if (args != null && args.length == 2) {
					String filePath = FileSys.getResolvedPath(fileSys.getResultantPath(args[1]));
					if (!FileSys.deleteFile(filePath))
						printError(CFunc.getString(R.string.error_cant_del) + filePath);
				}
				else {
					HelpCmd.printUsage(this, builtInCmd);
				}
				...
				if (args != null && args.length == 2) {
					...
					if (FileSys.fileExists(dirPath, false)) {
						printError(dirPath + CFunc.getString(R.string.error_path_exists));
					}
					else {
						if (!FileSys.mkdir(dirPath))
							printError(CFunc.getString(R.string.error_create_dir) + dirPath);
					}
				}
				else {
					HelpCmd.printUsage(this, builtInCmd);
				...
				if (args != null && args.length == 3) {
					String srcPath = FileSys.getResolvedPath(fileSys.getResultantPath(args[1]));
					String destPath = FileSys.getResolvedPath(fileSys.getResultantPath(args[2]));
					if (!FileSys.rename(srcPath, destPath))
						printError(String.format(CFunc.getString(R.string.error_cant_ren), srcPath, destPath));
				}
				else {
					HelpCmd.printUsage(this, builtInCmd);
				}
				...
				if (args != null && args.length == 3) {
					...
					do {
						if (!FileSys.fileExists(srcPath, true)) {
							printError(srcPath + CFunc.getString(R.string.error_notfile_notexist));
							break;
						}
						
						if (FileSys.isDir(destPath)) {
							String fileName = FileSys.getFileNameFromFullPath(srcPath);
							destPath = FileSys.combinePathAndFile(destPath, fileName);
						}
						
						if (!FileSys.copy(srcPath, destPath)) {
							printError(String.format(CFunc.getString(R.string.error_cant_cp), srcPath, destPath));
							break;
						}
				...
				String cmdString = cmdSettings.getCommandById(cmdInfo.getHistoryID());
				if (cmdString != null) {
				...
				if (args != null) {
				   if (args.length == 1) {
					  ...
					  try {
						 ConstantData.ConsoleFontSize fontSize = Enum.valueOf(ConstantData.ConsoleFontSize.class, args[1]);
						 uiSideCmd.setFontSize(fontSize.v());
						 cmdSettings.setValue(CmdSettings.Setting.console_font_size, fontSize.v());
					  }
					  catch (Exception e) {
				...
				if (!CFunc.isNullOrEmpty(cmdInfo.getOrgCmd()))
				   printError(CFunc.getString(R.string.error_unknown_cmd) + cmdInfo.getOrgCmd());
			}
